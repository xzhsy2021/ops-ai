            ...
            <table className="cc-table inspection-server-table">
              <thead>
                <tr>
                  ...
                  <th style={{ minWidth: 180 }}>服务器</th>
                  <th style={{ minWidth: 140 }}>IP / 主机</th>
                  <th style={{ width: 80 }}>状态</th>
                  <th style={{ minWidth: 100 }}>分组</th>
                </tr>
              </thead>
              <tbody>
        ...
        <section className="panel-card">
          ...
          <div className="inspection-ledger-layout">
            <div className="inspection-report-primary">
              ...
              {inspectionReports.length === 0 ? <EmptyState title="暂无巡检报表" description="生成单次/合并/周期巡检报告后会在这里分页展示。" /> : <><div className="table-scroll" style={{ marginTop: 8 }}><table className="data-table"><thead><tr><th style={{ width: 32 }}><input type="checkbox" checked={inspectionReports.length > 0 && inspectionReports.every((r: any) => selectedReportIds.includes(r.id))} onChange={(e) => setSelectedReportIds(e.target.checked ? inspectionReports.map((r: any) => r.id) : [])} /></th><th style={{ width: 140 }}>生成时间</th><th style={{ minWidth: 200 }}>标题</th><th style={{ minWidth: 180 }}>对象</th><th style={{ width: 80 }}>格式</th><th style={{ width: 80 }}>大小</th><th style={{ width: 80 }}>状态</th><th style={{ width: 80 }}>操作</th></tr></thead><tbody>{inspectionReports.map((r: any) => <tr key={r.id}><td><input type="checkbox" checked={selectedReportIds.includes(r.id)} onChange={(e) => toggleReportSelection(r.id, e.target.checked)} /></td><td><span className="ellipsis" style={{ maxWidth: 130 }} title={formatTime(r.created_at || r.generated_at)}>{formatTime(r.created_at || r.generated_at)}</span></td><td><span className="ellipsis" style={{ maxWidth: 280 }} title={r.title || r.id}>{r.title || r.id}</span></td><td><span className="ellipsis" style={{ maxWidth: 220 }} title={r.target_id || '-'}>{r.target_id || '-'}</span></td><td>{r.format || '-'}</td><td>{r.size_bytes ? `${Math.ceil(Number(r.size_bytes) / 1024)} KB` : '-'}</td><td><StatusBadge value={r.status || 'ready'} /></td><td><button className="btn btn-danger" onClick={() => deleteInspectionReports([r.id])}>删除</button></td></tr>)}</tbody></table></div><PaginationControls total={inspectionReportsTotal} limit={reportPageSize} offset={reportOffset} onChange={setReportOffset} onPageSizeChange={setReportPageSize} /></>}
            </div>
