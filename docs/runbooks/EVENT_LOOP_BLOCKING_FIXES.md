...
- 存活探测：`GET /healthz` → 200 `{"status":"ok"}`；`GET /readyz` → 200（含 DB 检查）；
  `GET /api/v2/health` → 401（鉴权正常）
  - 更正（2026-09-12 复盘第 10 轮）：此前这里写的是 `GET /health` → 200。实测 `GET /health`
    当时**并没有注册**，命中前端 SPA/登录页路由，未登录时 307 跳 `/login` 后返回 **200 HTML**
    ——无论后端是否健康都"200 就绪"，是假信号（已登录访问则直接 404 JSON）。
    现在 `/health` 已注册为与 `/healthz` 等价的 JSON 存活探测，但**推荐继续用 `/healthz`**。
