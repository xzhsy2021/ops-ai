...
7. **删除功能必须同步清理引用**：删除/重命名任何端点、工具、服务或配置结构后，
   提交前必须 `git grep` 全库检索已删符号（含 `app/`、`tests/`、`docs/`），
   同步删除或更新所有残留引用（测试用例、死代码、文档描述），确保无契约不一致。
   典型漏改点：测试参数化里的旧路径/旧函数、不可达的 else 分支、服务级路由等
   已收敛配置的旧断言。
8. **不动并行 Agent 的在途文件**：看到未提交的他人改动文件（`git status` 中
   `app/config/systems.py`、`app/api/approvals.py` 等），除非任务明确要求，否则
   不修改、不提交，避免覆盖并行工作；只处理与自身任务相关的文件。
