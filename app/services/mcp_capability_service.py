...
MCP_TOOL_DESCRIPTION_OVERRIDES: Dict[str, str] = {
    ...
    # ── Approval Workflow ──
    'ops.approval.prepare_plan': "Create an immutable execution plan for an entire Element message flow and return a one-time approval short token. After the approver approves once, all steps run in order automatically. Same plan content (plan_digest) reuses the pending plan idempotently. 中文: 准备执行计划/发起审批/部署审批/计划审批.",
    'ops.approval.execute_plan': "Consume a one-time approval short token, mark the execution plan ready, and run its frozen steps in order. The security gate is the token itself (one-time, 15-min expiry, bound to room+event), so only ops:read is needed. 中文: 执行审批计划/消费审批码/确认执行.",
    ...
    'ops.approval.prepare_service_control': 'Create an immutable approval ticket for service control (restart/stop/start/update) and return a one-time approval short token. An approver replies 批准 <短码> in the bound room. 中文: 准备服务控制审批/重启审批/停止审批/服务操作审批.',
    'ops.approval.temporary_access': 'Create/confirm/revoke bounded time-limited self-approval for test-environment changes. Identity is derived only from message_context.sender_id; only the configured original approver can confirm/revoke. 中文: 临时授权/临时审批/测试环境自批授权.',
    'ops.approval.list': 'List approval tickets with optional status and action-type filters. Read-only. 中文: 查看审批列表/审批记录/待审批/审批历史.',
    'ops.approval.execute': 'Consume an approval short token, mark the ticket EXECUTING, and trigger the real action (deploy/rollback/DML/package cleanup). Security is the token itself, so only ops:read is required. 中文: 执行已审批操作/消费审批码/确认操作.',
