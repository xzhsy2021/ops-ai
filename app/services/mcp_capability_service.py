...
MCP_TOOL_DESCRIPTION_OVERRIDES: Dict[str, str] = {
    ...
    'ops.approval.reject': "Officially mark a pending (PENDING_APPROVAL) single-action ticket (SERVICE_CONTROL / EXEC_REMOTE / FILE_UPLOAD) as REJECTED so it can no longer be approved and executed. Complement of ops.approval.reject_plan, which handles execution plans only. Idempotent: a ticket already in a terminal state is returned as-is with no side effect; only PENDING_APPROVAL can be rejected. Requires ops:write; the rejecter identity is recorded as rejected_by for audit, and an optional reason is persisted to failure_reason. 中文: 拒绝工单/拒绝单动作工单/驳回命令执行审批/取消待审批工单/标记已拒绝.",
