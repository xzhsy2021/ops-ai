...
export default function FileCenterPage() {
  ...
  return (
    <div className="file-page-shell">
      ...
      {activeTab === 'packages' && (
        <div className="file-cleanup-grid">
          <section className="glass-card">
            ...
            {loadingPkg ? (
              ...
              <div style={{ color: 'var(--text-muted)', marginTop: '16px' }}>暂无部署包，请先上传本地构建产物。</div>
            ) : (
                ...
                <table className="file-table">
                  ...
                  <tbody>
                    {packages.map((p) => {
                      ...
                      return (
                        <tr key={p.name}>
                          <td><span className="file-name">{p.name || p.package_name}</span></td>
                          <td><span className="file-size">{p.size_mb} MB</span></td>
                          <td>
                            <span className="file-name">{p.service_hint || '-'}</span>
                            <div className="file-meta">{p.version_hint || '-'}</div>
                          </td>
                          <td>
                            <span className="file-meta">{p.used_count || 0} 次</span>
                            <div className="file-meta">{p.last_used_at?.slice(0, 16) || '未使用'}</div>
                          </td>
                          <td>
                          ...
                          <td>
                            {checksums[p.name] ? (
                              <span className={`file-checksum ${checksums[p.name] === '计算失败' ? 'file-checksum--err' : 'file-checksum--ok'}`}>{checksums[p.name]}</span>
                            ) : p.sha256 ? (
                              <span className="file-checksum">{String(p.sha256).slice(0, 12)}…</span>
                            ) : (
                              <button className="cc-icon-btn" onClick={() => calcChecksum(p.name)} disabled={checking[p.name]}>
                                {checking[p.name] ? '…' : '校验'}
                              </button>
                            )}
                          </td>
          ...
          <section className="glass-card file-upload-card">
            <h2 style={{ margin: 0, fontFamily: 'var(--font-display)', fontSize: 16, fontWeight: 600, letterSpacing: '-0.015em' }}>上传本地包</h2>
            ...
            <button className="cc-icon-btn cc-icon-btn--success" style={{ height: 32, fontSize: 11.5 }} onClick={() => fileInputRef.current?.click()} disabled={uploading}>
              {uploading ? '上传中…' : '选择并上传'}
            </button>
      ...
      {activeTab === 'cleanup' && (
        <div className="file-cleanup-grid">
          <section className="glass-card file-upload-card">
            <h2 style={{ margin: 0, fontFamily: 'var(--font-display)', fontSize: 16, fontWeight: 600, letterSpacing: '-0.015em' }}>发布包清理策略</h2>
            <p style={{ margin: '4px 0 0', color: 'var(--text-muted)', fontSize: 13, lineHeight: 1.7 }}>
              清理基于引用关系，不会删除运行中、失败重试期、回滚候选和最近成功发布依赖的包。
            </p>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <input type="checkbox" checked={!!retention.protect_running_deployments} onChange={(e) => updateRetentionField('protect_running_deployments', e.target.checked)} /> 保护运行中发布包
            </label>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <input type="checkbox" checked={!!retention.protect_failed_deployments} onChange={(e) => updateRetentionField('protect_failed_deployments', e.target.checked)} /> 保护失败重试期包
            </label>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <input type="checkbox" checked={!!retention.protect_rollback_candidates} onChange={(e) => updateRetentionField('protect_rollback_candidates', e.target.checked)} /> 保护回滚候选包
            </label>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button className="cc-icon-btn" onClick={loadRetention}>刷新策略</button>
              <button className="cc-icon-btn cc-icon-btn--success" onClick={saveRetention}>保存策略</button>
              <button className="cc-icon-btn cc-icon-btn--success" onClick={previewCleanup} disabled={cleanupBusy}>清理预览</button>
            </div>
          </section>
          ...
          <section className="glass-card">
            <div className="section-title-row">
              <div>
                <h2>清理预览</h2>
                <p>真实删除前请先确认候选和保护原因</p>
              </div>
              <button className="cc-icon-btn cc-icon-btn--danger" onClick={() => setCleanupConfirmOpen(true)} disabled={cleanupBusy || !cleanupPreview?.summary?.cleanup_count}>
                按预览清理
              </button>
            </div>
            ...
            {cleanupPreview ? (
              <>
                ...
                <h3 style={{ margin: '18px 0 8px', fontFamily: 'var(--font-display)', fontSize: 13, fontWeight: 600, color: 'var(--text-strong)' }}>候选包</h3>
                    ...
                    <tbody>
                      {(cleanupPreview.candidates || []).map((p: any) => (
                        <tr key={p.package_name}>
                          <td><span className="file-name">{p.package_name}</span></td>
                          <td><span className="file-size">{p.size_mb} MB</span></td>
                          <td><span className="file-meta">{p.reason}</span></td>
                        </tr>
                      ))}
              ...
              <div style={{ color: 'var(--text-muted)', marginTop: 16 }}>点击"清理预览"查看候选包和保护原因。</div>
            )}
          </section>
        </div>
      )}
